...
class GTVisualizer():
    ...
    def update_vis(self):
        if not self.is_updated:
            ...
            self.amodal, self.vis = self.visualize_masks()
        ...
        rgbd = np.hstack((self.rgb, self.depth))
        masks = np.hstack((self.amodal, self.vis))
        graphs = np.hstack((self.occ_graph, self.depth_graph))
        self.frame = np.vstack((rgbd, masks, graphs))
    ...
    def load_rgbd(self):
        ...
        rgb = cv2.resize(rgb, (self.width//2, self.height//2), interpolation=cv2.INTER_NEAREST)
        depth = cv2.resize(depth, (self.width//2, self.height//2), interpolation=cv2.INTER_NEAREST)
        return rgb, depth
    ...
    def visualize_masks(self):
        ...
        for idx, anno in enumerate(self.annotations):
            ...
            amodal_mask = cv2.resize(amodal_mask, (self.width//2, self.height//2), interpolation=cv2.INTER_NEAREST)
            ...
            vis_mask = cv2.resize(vis_mask, (self.width//2, self.height//2), interpolation=cv2.INTER_NEAREST)
        ...
        for i, (amodal_mask, vis_mask, amodal_topleft, vis_topleft) in enumerate(zip(amodal_masks, vis_masks, amodal_toplefts, vis_toplefts)):
        ...
        amodal = cv2.resize(amodal, (self.width//2, self.height//2), interpolation=cv2.INTER_NEAREST)
        vis = cv2.resize(vis, (self.width//2, self.height//2), interpolation=cv2.INTER_NEAREST)
        return amodal, vis
    ...
    def visualize_graphs(self):
        ...
        occlusion_graph = cv2.resize(occlusion_graph, (self.width//2, self.height//2), interpolation=cv2.INTER_NEAREST)
        plt.cla()
        plt.clf()
        ...
        depth_graph = cv2.resize(depth_graph, (self.width//2, self.height//2), interpolation=cv2.INTER_NEAREST)
        plt.cla()
        plt.clf()
        lock.release()
